...
$probeName = if ($MtrOnly) { 'runtime-mtr-only-011' } elseif ($BaseOnly) { 'runtime-base-011' } elseif ($Connector) { 'runtime-connector-011' } else { 'runtime-probe-011' }
...
Copy-Item -LiteralPath "$project/build/libs/point-runtime-probe-0.1.1.jar" -Destination "$game/mods" -Force
...
Copy-Item -LiteralPath "$project/build/libs/mtr_railway_point_advanced-0.1.1.jar" -Destination "$game/mods" -Force
...
[IO.File]::WriteAllText("$game/options.txt", "lang:zh_cn`nguiScale:2`nrenderDistance:4`n", $utf8)
