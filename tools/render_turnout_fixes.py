"""Inspect curved V bearers and asymmetric turnout wing alignment with Blender."""
import bpy
from pathlib import Path
from mathutils import Vector
root=Path(__file__).resolve().parents[1]/'build/previews'
for name,center,scale in [('asymmetric-curved',(2,10,0),24),('scissors-curved',(2,0,0),44)]:
    ...
    scene.render.filepath=str(root/(name+'-bearer-fix.png'));bpy.ops.wm.save_as_mainfile(filepath=str(root/(name+'-bearer-fix.blend')));bpy.ops.render.render(write_still=True)
