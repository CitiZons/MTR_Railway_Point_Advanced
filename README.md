...
**2026-09-18 几何修复：** 相互覆盖的道岔不再分别提交固定走行轨；固定钢轨在生成时记录曲线和里程区间，世界级装配在轨道或外观变化时预编译全部切区，渲染帧只读取缓存，防止另一模型把断口补回并避免实时重复裁切。外侧分支从节点中心沿自身曲线连续生成。短距离重叠的护轨按未外撇工作中心线合并，仅保留整根护轨最外两端的外撇，并补齐自定义轨型原先缺失的端面。尖轨按基本轨工作边对齐，仅在实际贴靠基本轨的头部短段从基本轨一侧刨切，后段保留完整正常截面，不再将整个截面对称缩窄。岔区选择图现可预览、选中、拖动、撤销并直接保存全部枕木。程序回归、重混淆构建和隔离世界运行探针均已通过；真实存档中的全部第三方轨型组合仍以实景复核为准。版本保持 0.1.2。

**2026-09-21 修复：** 护轨现在全部可选中、可合并：交点（平交）与三开的护轨也纳入选择，选中后按各自股道延长并写入同一合并组，合成一根时提示"护轨已合并为一根"，未能合成一根时提示"所选护轨不在同一条线上，已按各自股道延长"（不再直接拒绝）；只有尖轨旁的基本轨不可选。选中判定改按**来源**（`GuardRails.merge` 回报每条并合轨由哪些输入得到）而非几何邻近，因此平交处单击不会再顺带点亮相交的其他护轨。合并后的护轨在内部接缝处不再留下端面（`path`／`convert` 只在露出的端头画端面），并且会被相交的走行轨切断：原先 `DiamondGeometry.bakeLayer` 对 guard 断面用 5 mm 等高判定，而检测器允许平交两条道路相差 80 mm，于是那类平交的护轨直接穿过钢轨；现在只要两根钢轨的截面在竖直方向可能重叠就必须切断，只有高出一整个截面才让下方护轨完整保留。几何合并本身仍只把同侧同一根钢轨并成一根，并只保留并集最外两端的外撇；编辑器预览继续画出合并后的护轨。切区方面，`RailCuts` 原先要求断面、横向位置与竖向偏移都完全相等才认领一段钢轨，任一相邻道岔改了轨距／轨头宽度／轨顶高度／竖向偏移后，另一视图的辙叉断口就会落空、断口被连续钢轨跨过；现在按"钢轨在道路的哪一侧"认领，断面与高度不再作为门槛。原先无人引用的 `RailSection` 改为以逐层真实交点约束切区，回归新增 54 段交点区间与"外观覆盖邻段必须被切断"的断言。`DiamondGeometry` 的最宽层 span 宽度保持原状，仍待游戏内复核，详见 `VALIDATION.md`。

**2026-09-21 性能优化：** 客户端方面，列车运动快照不再每 tick 遍历每辆车的整条路径（改为每 4 tick），`Detector` 的每秒全量识别不再为每个空间格和每对线段拼接字符串（改为 long 键并靠交点坐标去重），`RailSampler` 命中缓存时不再白做 `writePositions` 与两次反射，编辑器输入不再每个按键/拖拽事件重建全部视图几何（改为标脏后每帧合并一次）。服务端方面，BR 快照读取不再每访问一个字段就反射查找一次方法（按类+方法名缓存），运动快照不再每 4 tick 把整份（最多 8192 条）广播给整个维度（改为按玩家 256 格过滤，空表也发以维持心跳），进服/切换维度不再每个外观条目发一个包（合成为一个批次包，频道版本升至 4）。详见 `VALIDATION.md`。

**2026-09-21（续）撤回与现状记录：** 尖轨（switch blade）的刨切／变尖渲染改动已**撤回**——把尖轨按横截面横向缩放变尖的改法让渲染更差，现恢复为改动前的实现：先生成完整原生截面，仅在仍与基本轨接触的短段用 `clipAnimated` 从基本轨一侧按竖直刨切面裁切，`TurnoutFrame.Blade` 恢复为 `(point,cut)`，回归断言同步改为"按原生截面竖直刨切、不再变尖"并通过。以下问题按用户要求**不再做修改，按现状记录**：①钢轨与护轨的切断端面已改为取原生模型 zMin 轮廓与自身 surface，但游戏内看仍然没有材质（待查）；②合并护轨时，中间那根护轨的终点有时会按预期 cancel out、有时有一个做不到，而合并完成后外侧两端的终点反而被 cancel out（与预期相反）；③护轨偶尔仍有未按平交口切断的情况（"风格组不匹配则永远不被切"已修，仍有个别漏切，缺少具体场景无法继续）。门禁 `.\gradlew.bat regression build smokeJar --no-daemon --console=plain` 通过（51 条 PASS 断言、Forge 编译与重混淆打包）。
...
- 按 P 打开编号岔区选择图，可查看附近全部道岔及其枕木；可直接选中并沿所属轨道拖动单根枕木、预览和保存，也可进入所选道岔的独立蓝图。不增加物品或方块。
- 单道岔“枕木排列”页可选择曲线参考路径、点击轨道手动补加枕木，并对选中的单根枕木执行删除、切断／重新连接、恢复完整形状／恢复自动裁边；“仅重置枕木”不会改动钢轨、辙叉或轨型参数。
...
- 基本轨与尖轨按实际工作边对齐；尖轨先生成完整轨型，只在贴靠基本轨的短段用基本轨一侧的竖直刨切面去除干涉部分，离开接触区后恢复完整轨头、轨腰和轨底。
- 世界渲染统一装配相互覆盖的固定走行轨与护轨；所有相邻单开道岔的辙叉断口共同裁切固定轨，护轨的同轨区间及局部实体重叠均移除内部喇叭口。相邻道岔编辑器仍独立；修改后重新计算，静止外观不会逐帧重建合并网格。
- 相邻菱形交叉在同一固定组件内生成，任一交叉的轮缘槽都会裁切区域内所有重叠走行轨与翼轨；自定义立体轨型绘制的护轨和翼轨在暴露端补齐带钢轨贴图 UV 的截面，不再呈现空心端口。
...
4. 左侧是实际生成网格的俯视／轴测预览。滚轮缩放、右键拖动平移；点击岔枕任意位置后拖动可调整该根岔枕的纵向位置。岔区选择图也支持同样操作，并提供撤销和直接保存。
...
6. “枕木排列”页默认使用 V 形：两臂分别与自己的支线正交，在中间裁切拼接；首尾偏角可调整整体偏转或沿程变化。另保留沿曲线加整体偏角、全部平行、首尾线性渐变、首尾平滑渐变，并可切换这些模式采用的参考路径。点“添加枕木”后可在预览轨道上指定位置；选中单根枕木后可删除、切断／连接，或在完整形状与自动裁边之间切换。“仅重置枕木”清除这些编辑并恢复枕木默认参数，不影响其他外观设置。
...
蓝图预览改用一次线段批次，移除逐像素 `fill`；投影基向量按帧计算，边列表按模型变化缓存。试动不再逐帧完整重建道岔，动画只插值活动钢轨顶点。轨道采样按 Rail / Optional Rail 节点设置复用；最近点查询使用空间树；替换判定按所属轨道索引；行车数据按节点索引。平交、辙叉切区和护轨并集在轨道拓扑或外观变化后的客户端更新阶段生成一次，渲染回调不再读取位置并重复计算。世界模型按材质上传到持久 GPU 缓冲，固定与活动部件分开，使用视锥裁剪；块光按间隔检查，变化才重新上传。重用原生内存暂存区，清理离开检测范围的 GPU 缓冲；交叉渡线分组按几何及启用状态缓存。全屏蓝图和选择图打开时跳过被遮住的本道岔世界模型提交。
