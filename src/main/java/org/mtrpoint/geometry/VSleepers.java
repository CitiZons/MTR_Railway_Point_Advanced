...
public final class VSleepers {
    ...
    public static void across(Mesh out,List<Track> tracks,V3 spine,V3 forward,double angle,PointSettings s,Profile p,int index){
        ...
        for(int i=0;i<arms.size();i++){
            ...
            if(!SleeperEdits.split(s,index)&&!SleeperEdits.full(s,index))for(int side:new int[]{-1,1}){V3 limit=side<0?left:right;if(limit==null)continue;
    ...
    public static void add(Mesh out,Junction j,PointSettings s,Profile p,double d,double otherDistance,double extent,int index){
        ...
        for(int branch=0;branch<2;branch++){
            ...
            if(SleeperEdits.split(s,index)||SleeperEdits.full(s,index))out.quads.addAll(arm.quads);else for(var q:arm.quads)Mesh.clip(out,q,joint,branch==0?cut:cut.mul(-1));
