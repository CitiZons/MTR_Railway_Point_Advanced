...
final class PointGpu implements AutoCloseable {
    ...
    void update(Mesh mesh,double position,boolean movable,boolean removeGuards,boolean removeSharedCrossing){
        ...
        if(changed){
            ...
            for(int i=0;i<mesh.quads.size();i++){
                var q=mesh.quads.get(i);if(removeGuards&&(q.part().equals("guard")||q.part().equals("sleeper")||q.part().equals("fastener")||q.part().equals("wing")||q.part().equals("rail"))||removeSharedCrossing&&q.part().equals("frog"))continue;
    ...
    private void upload(List<Batch> target,List<Integer> ids){
        ...
        for(var entry:groups.entrySet()){
            ...
            for(int id:entry.getValue()){
                ...
                int color=partColor(q.part(),s.color());
                for(int i=0;i<4;i++){
                    ...
                    b.vertex(v.x()-origin.x(),v.y()-origin.y(),v.z()-origin.z()).color(color).uv(u,vv).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(light).normal((float)n.x(),(float)n.y(),(float)n.z()).endVertex();vertices++;
                }
            }
